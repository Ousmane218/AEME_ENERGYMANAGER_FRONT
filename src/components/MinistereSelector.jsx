...
import { Search, Landmark, MapPin, Loader2, X } from 'lucide-react';
import { getAllMinisteres } from '../services/referenceService';
import { cn } from "@/lib/utils";

/**
 * A searchable select component for choosing a Ministere
 */
export const MinistereSelector = ({ onSelect, selectedId = null, className = "" }) => {
    ...
    useEffect(() => {
        const fetchMinisteres = async () => {
            try {
                ...
                setMinisteres((data || []).filter(m => m.actif !== false));
            } catch (err) {
                console.error("Failed to fetch ministeres", err);
            } finally {
                setLoading(false);
            }
        };
    ...
    useEffect(() => {
        if (selectedId && ministeres.length > 0) {
            const found = ministeres.find(s => String(s.id) === String(selectedId));
            if (found) setSelectedMinistere(found);
        }
    }, [selectedId, ministeres]);
    ...
    const filtered = ministeres.filter(s =>
        s.nom?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        s.code?.toLowerCase().includes(searchTerm.toLowerCase())
    );

    const handleSelect = (ministere) => {
    ...
    return (
        <div className={cn("relative w-full", className)} ref={wrapperRef}>
            ...
            >
                <Landmark size={18} className={cn("shrink-0 transition-colors", selectedMinistere ? "text-primary" : "text-gray-400")} />
                <div className="flex-1 min-w-0">
                    {selectedMinistere ? (
                        <div className="flex flex-col">
                            <span className="text-[11px] font-black text-gray-900 truncate leading-tight uppercase tracking-tight">
                                {selectedMinistere.nom}
                            </span>
                            <span className="text-[9px] text-gray-400 font-bold uppercase tracking-widest truncate">{selectedMinistere.code}</span>
                        </div>
                    ) : (
                        <span className="text-sm text-gray-400">Rechercher une ministere...</span>
                    )}
                </div>
                    ...
                    <div className="max-h-60 overflow-y-auto">
                        {filtered.length === 0 ? (
                            <div className="p-8 text-center text-gray-400">
                                <p className="text-xs font-bold">Aucune ministere trouvée</p>
                            </div>
                        ) : (
                            filtered.map((s) => (
                                <div
